...
**Как работает:**
1. Клиент мгновенно обнаруживает потерю сети (EADDRNOTAVAIL / NetworkUnreachable) — без ожидания keepalive timeout
2. Обновляет маршрут к VPN-серверу через новый default gateway (`update_server_route`)
3. Создаёт новый UDP socket и отправляет зашифрованный "roam ping"
4. Сервер пытается расшифровать пакет ключами каждого клиента (`can_decrypt`)
5. При успехе — обновляет endpoint клиента, трафик продолжается
6. Если soft roam не удался — клиент делает полный re-handshake
